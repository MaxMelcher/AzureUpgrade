...
export class ExportService {
  public async copyResults(results: RecommendationResult[], currencyCode: string): Promise<void> {
    const rows = [
      ['Current VM', 'Recommended VM', 'Status', 'Current hourly', 'New hourly', 'Monthly saving', 'Confidence'],
      ...results.map((result) => [
  ...
  public downloadResults(
    ...
    const rows: Array<Array<string | number | null>> = [[
      'Current VM', 'Recommended VM', 'Region', 'OS', 'Status', 'Current vCPU', 'New vCPU',
      'Current RAM GB', 'New RAM GB', 'CPU vendor', 'Temp disk MB', 'Current hourly',
      'New hourly', 'Current yearly', 'New yearly', 'Monthly saving', 'Saving %',
      'Currency', 'Confidence', 'Explanation'
    ]];
  ...
  public downloadQualityMatrix(
    ...
    const rows: Array<Array<string | number | null>> = [[
      'Region', 'Source VM', 'OS', 'CPU policy', 'Status', 'Recommended VM',
      'Source hourly', 'Recommended hourly', 'Monthly saving', 'Saving %',
      'Currency', 'Confidence', 'Explanation'
    ]];
    for (const row of matrix) {
      rows.push([
        row.region, row.sourceSku, row.os, row.cpuPolicy, row.status, row.recommendation,
        row.sourceHourly, row.recommendedHourly, row.monthlySaving, row.savingPercent,
        currencyCode, row.confidence, row.explanation
      ]);
    }
    this.download(rows, fileName);
  }

  private price(result: RecommendationResult, vm: NonNullable<RecommendationResult['source']>): number | null {
